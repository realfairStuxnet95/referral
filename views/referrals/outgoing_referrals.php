...
if($_SERVER['REQUEST_METHOD'] == 'POST'){
	...
	if($action=="select"){
		$options=$function->sanitize($data[1]);
		$status=$function->sanitize($data[2]);
		displayResult($referral,$function,$hos_id,$options,$status,0,"");
	}elseif($action=="search"){
		$options=$function->sanitize($data[1]);
		$input=$function->sanitize($data[2]);
		displayResult($referral,$function,$hos_id,$options,"",$input,$input);
	}else{
		displayResult($referral,$function,$hos_id,4,"",0,"");
	}
...
function displayResult($referral,$function,$from_hospital_id,$options,$status,$number,$input){
	$referrals=$referral->ongoing_referrals($from_hospital_id,$options,$status,$number,$input);
	include 'util/add.php';
